...
export const Analysis = () => {
  ...
  const jobTitle = location.state?.jobDescription || 'Target Job Profile';
  ...
  const handlePayment = async () => {
    try {
      if(!user) return toast.error('You must log in to upgrade.');
      const { data } = await axios.post('http://localhost:5002/api/payment/create-order', 
        { amount: 499 },
      ...
      const options = {
        ...
        currency: 'INR',
        name: 'SkillGap AI Premium',
        description: 'Unlock detailed AI Roadmaps',
        order_id: data.order.id,
        handler: async function (response) {
          try {
            const verifyRes = await axios.post('http://localhost:5002/api/payment/verify', {
            ...
            if (verifyRes.data.success) {
              ...
              // For demo purposes, we automatically navigate to roadmap for Clerk
              navigate('/roadmap', { state: { roadmapHtml: analysisData?.learningRoadmap } }); 
            }
          } catch (error) { toast.error('Payment verification failed'); }
        },
        prefill: { name: user?.firstName || 'Guest', email: user?.primaryEmailAddress?.emailAddress || '' },
        theme: { color: '#5D3DEB' },
      };
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div className="flex justify-between items-end" style={{ marginBottom: '2.5rem' }}>
          ...
          <div className="flex gap-4">
            <button className="btn" style={{ background: '#F1F5F9', color: 'var(--text-primary)' }}>Share Report</button>
            <button className="btn" style={{ background: '#F1F5F9', color: 'var(--text-primary)' }}>Export PDF</button>
          </div>
        </div>
        ...
        <div style={{ background: 'linear-gradient(135deg, #4F46E5 0%, #8B5CF6 100%)', borderRadius: '24px', padding: '3rem', color: 'white', marginTop: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {isPremium ? (
             <button onClick={() => navigate('/roadmap', { state: { roadmapData: analysisData?.learningRoadmap } })} className="btn" style={{ background: 'white', color: 'var(--primary-color)', padding: '1rem 2rem', fontSize: '1.125rem', fontWeight: 700 }}>Generate Learning Roadmap →</button>
          ) : (
             <button onClick={() => navigate('/roadmap', { state: { roadmapData: analysisData?.learningRoadmap } })} className="btn" style={{ background: 'white', color: 'var(--primary-color)', padding: '1rem 2rem', fontSize: '1.125rem', fontWeight: 700 }}>Unlock Learning Roadmap →</button>
          )}
        </div>
