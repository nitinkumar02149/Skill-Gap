    ...
    <Layout>
      <div style={{ maxWidth: '1000px' }}>
        ...
        <div className="flex-col gap-4">
          {[
            { title: 'Principal Product Designer - Stripe', date: 'Analyzed 2 days ago • 82% Match', tag: 'HIGH MATCH', color: '#10B981', bg: '#ECFDF5' },
            { title: 'Lead UX Architect - Airbnb', date: 'Analyzed 5 days ago • 74% Match', tag: 'GAP IDENTIFIED', color: '#F59E0B', bg: '#FFFBEB' },
            { title: 'Senior Design Systems Engineer - Vercel', date: 'Analyzed 1 week ago • 68% Match', tag: 'CRITICAL GAPS', color: '#6B7280', bg: '#F3F4F6' }
          ].map((item, i) => (
            <div key={i} className="card flex items-center justify-between" style={{ padding: '1.25rem 1.5rem' }}>
               <div className="flex items-center gap-4">
                 ...
                 <div>
                   <div style={{ fontWeight: 600, color: 'var(--text-primary)', fontSize: '0.9rem', marginBottom: '0.25rem' }}>{item.title}</div>
                   <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>{item.date}</div>
                 </div>
               </div>
               <div className="flex items-center gap-4">
                 <span className="pill" style={{ background: item.bg, color: item.color, fontSize: '0.65rem', letterSpacing: '0.05em' }}>{item.tag}</span>
